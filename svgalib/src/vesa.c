...
struct vesa_mode_mapping
{
	int mode8;
	int mode16;
};

static struct vesa_mode_mapping vesa_modes[] = {
	{10, 18},
	{11, 21},
	{12, 24},
	{39, 41},
...
{ 
	/* We only want to go back to text mode */
/*	void * buf;
	buf=LRMI_mem1;
	vesa_r.eax=0x4f04;
	vesa_r.ebx=0;
...
static int vesa_setmode(int mode, int prv_mode)
{
	...
	while (vesa_modes[i].mode8 != -1) {
		if (vesa_modes[i].mode8 == mode) {
			/* Promote the mode to 16-bit. Thrice accursed
			   non-palette supporting hardware */
			mode8 = mode;
			mode = vesa_modes[i].mode16;
			break;
		}
		i++;
	}
	...
	if (mode == 0) {
	  ...
	  if ((vesa_r.eax & 0xff00) != 0) {
	    /* We appear to have failed to set a 16-bit mode. Ho-hum.
	       Try setting an 8-bit one instead. */
	    if (mode8)
	      mode = vesa_setmode (mode8, prv_mode);
	    else
	      return -2;
	  }
...
{  
    ...
    m=mmap(0,1024,PROT_READ,MAP_SHARED,__svgalib_mem_fd,0);
    
    address=((long)*(unsigned short *)(m+64))+(((long)*(unsigned short *)(m+66))<<4);
        if((address<0xa0000)||(address>0xfffff)) {
           	fprintf(stderr, "Real mode int 0x10 handler not in ROM.\n");
    ...
    if (strncmp(vesa_data.info->vbe_signature, "VESA", 4) != 0) {
       	fprintf(stderr,"No VESA bios detected!\n");
        fprintf(stderr,"%04x %i %i %i %i\n",vesa_r.eax,
        vesa_data.info->vbe_signature[0],
        vesa_data.info->vbe_signature[1],
        vesa_data.info->vbe_signature[2],
        vesa_data.info->vbe_signature[3]);
        fprintf(stderr,"Try running vga_reset.\n");
       	return 1;
    }
